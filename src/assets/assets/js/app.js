var app = {
	admin:{
		...
		ini2: function(row){
			// console.log(row);
			jQuery('#txRow').val(row);
			jQuery('#designer-products .modal-body a.modal-link').click(function(){
				// alert('1');
				console.log(row);
				var link = jQuery(this).attr('href');
				if(jQuery(this).hasClass('add-link'))
					app.admin.add_option(this, row);
				else
					app.admin.load2(link,row);
				return false;
			});
		},
		product: function(e, index){
			if (document.getElementById('designer-products') == null)
			{
				var div = '<div class="modal fade" id="designer-products" tabindex="-1" role="dialog" style="z-index:10520;" aria-labelledby="myModalLabel" aria-hidden="true">'
						...
						+	'</div>'
						+ '</div></div>'
						+ '<input type="hidden" id="txRow" value="-1" />';
		...
		product2: function(e, index, row){
			...
			if (document.getElementById('designer-products') == null)
			{
				var div = '<div class="modal fade" id="designer-products" tabindex="-1" role="dialog" style="z-index:10520;" aria-labelledby="myModalLabel" aria-hidden="true">'
						...
						+	'</div>'
						+ '</div></div>'
						+ '<input type="hidden" id="txRow" value="'+row+'" />';
				jQuery('body').append(div);
			}
			...
			jQuery.post(url, data, function(response) {		
				jQuery('#designer-products .modal-body').html(response);
				// jQuery('#designer-products .modal-body a.add-link').attr('href') = 'javascript:void(0)';
				app.admin.ini2(row);
			});
			return false; 
		},
		...
		add_option:function(e, row){
			console.log(row);
			var id = jQuery(e).data('id');
			var title = jQuery(e).data('title');
			var img = jQuery(e).children('img').attr('src');
			document.getElementById('_product_id'+row).value = id;
			
			// var price = document.getElementById('design_print_price_').value;
			
			document.getElementById('_product_title_img'+row).value = title +'::'+ img +'::'+ 0;
			
			var html = '<img src="'+img+'" class="img-responsive img-thumbnail" alt="'+title+'">';
			html = html + '<br /><center>'+title+'</center>';
			
			jQuery('#add_designer_product'+row).html(html);
...
function setHeigh(height){
	height = height + 10;
	document.getElementById('tshirtecommerce-designer').setAttribute('height',  '700px');
}
...
function viewBoxdesign(){
	...
	if (width < 700 || height < 600)
	{
		...
		jQuery('body').append('<div id="modal-design-bg"></div><div id="modal-designer"><a href="'+urlBack+'" class="btn btn-dange btn-xs">Close</a><iframe id="tshirtecommerce-designer" scrolling="no" frameborder="0" width="100%" height="700px;" src="'+url+'"></iframe></div>');
	}
	else
	{
		jQuery('.row-designer').html('<iframe id="tshirtecommerce-designer" scrolling="no" frameborder="0" noresize="noresize" width="100%" height="700px;" src="'+urlDesignload+'"></iframe>');
	}
}
