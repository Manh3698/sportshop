		...
		preview: function (e) {
			...
			setTimeout(function () {
				if (jQuery('#view-front .product-design').html() != '') {
					...
					// console.log(design.output);
					jQuery('#slide-front').append(design.output.front);

					var mycanvas = document.getElementById("canvastest"); //get your canvas
					var imagess = mycanvas.toDataURL("image/png");
					// jQuery.ajax({
					//   type: "POST",
					//   url: 'http://localhost/inan/index.php?route=common/home/saveImage',
					//   data: {image:imagess,},
					//   success: function(dataa){
					//   	window.location.replace("http://localhost/inan/index.php?route=common/home/preview&href="+dataa);
					//   },
					// });
					// console.log(imagess);
				}
				...
				jQuery.ajax({
					...
					if (data.arts.length > 0) {
						...
						while (start - i < limit) {
							...
							a.item = art;
							var img = '<img alt="" style="height:84px;" src="' + url + art.medium + '">';
							a.innerHTML = img;
							div.appendChild(a);
				...
				if (typeof obj[i] != 'undefined') {
					...
					if (layer.id != 'area-design') {
						...
						if (thumb.indexOf('http') != -1) {
							...
							imageObj.src = siteURL + 'image-tool/index.php?src=http://' + thumb + '&w=' + width + '&h=' + height;
						}
						else {
							imageObj.src = thumb;
						}
